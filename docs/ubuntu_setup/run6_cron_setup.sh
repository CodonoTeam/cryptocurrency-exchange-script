...
# ============================================
# Codono Exchange - Cron Job Setup
...
# Extract values from the credentials file
...
# Process optional selections (empty input is OK — mandatory crons still get installed)
if [ -n "$selection" ]; then
    for i in $selection; do
        if ! [[ "$i" =~ ^[1-6]$ ]]; then
            echo "Invalid selection: $i. Please enter valid numbers (1-6)."
            exit 1
        fi
...
crontab -u www-data "$CRON_TMP_FILE"
rm "$CRON_TMP_FILE"
...
echo "Cron jobs have been updated and reloaded. You can check crons using: crontab -l -u www-data"
